...
{
    public class GetAllEpvoStudentDumpQueryHandler : IRequestHandler<GetAllEpvoStudentDumpQuery, IReadOnlyList<EpvoStudentDumpDto>>
    {
        private readonly IEpvoSsoRepository<Student_Dump> _repository;

        public GetAllEpvoStudentDumpQueryHandler(IEpvoSsoRepository<Student_Dump> repository)
        {
            _repository = repository;
        }

        public async Task<IReadOnlyList<EpvoStudentDumpDto>> Handle(
            GetAllEpvoStudentDumpQuery request, CancellationToken ct)
        {
            var entites = await _repository.GetAllAsync(ct);
            return entites.Select(s => new EpvoStudentDumpDto
            {
