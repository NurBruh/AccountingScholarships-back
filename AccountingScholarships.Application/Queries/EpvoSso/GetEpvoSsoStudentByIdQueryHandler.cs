...
using MediatR;
using EpvoStudent = AccountingScholarships.Domain.Entities.epvosso.Student;

namespace AccountingScholarships.Application.Queries.EpvoSso;

public class GetEpvoSsoStudentByIdQueryHandler
    : IRequestHandler<GetEpvoSsoStudentByIdQuery, EpvoStudentSsoDto?>
{
    private readonly IEpvoSsoRepository<EpvoStudent> _repository;

    public GetEpvoSsoStudentByIdQueryHandler(IEpvoSsoRepository<EpvoStudent> repository)
    {
        _repository = repository;
    }

    public async Task<EpvoStudentSsoDto?> Handle(
        GetEpvoSsoStudentByIdQuery request, CancellationToken cancellationToken)
    {
        var s = await _repository.GetByIdAsync(request.Id, cancellationToken);
        if (s is null) return null;

        return new EpvoStudentSsoDto
        {
