...
public class GetStudentInfoByIdQueryHandler
    : IRequestHandler<GetStudentInfoByIdQuery, EpvoStudentInfoDto?>
{
    private readonly IEpvoSsoRepository<Student_Info> _repository;

    public GetStudentInfoByIdQueryHandler(IEpvoSsoRepository<Student_Info> repository)
    {
        _repository = repository;
    }

    public async Task<EpvoStudentInfoDto?> Handle(
        GetStudentInfoByIdQuery request, CancellationToken cancellationToken)
    {
        var s = await _repository.GetByIdAsync(request.Id, cancellationToken);
        if (s is null) return null;

        return new EpvoStudentInfoDto
        {
