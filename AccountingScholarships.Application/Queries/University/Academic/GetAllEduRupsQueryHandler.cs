...
public class GetAllEduRupsQueryHandler : IRequestHandler<GetAllEduRupsQuery, IReadOnlyList<Edu_RupsDto>>
{
    private readonly ISsoRepository<Edu_Rups> _repository;

    public GetAllEduRupsQueryHandler(ISsoRepository<Edu_Rups> repository)
    {
        _repository = repository;
    }

    public async Task<IReadOnlyList<Edu_RupsDto>> Handle(GetAllEduRupsQuery request, CancellationToken cancellationToken)
    {
        var entities = await _repository.GetAllWithIncludesAsync(
            new[] { "Algorithm", "EducationDuration", "Speciality" }, cancellationToken);

        return entities.Select(MapToDto).ToList().AsReadOnly();
    }

    private static Edu_RupsDto MapToDto(Edu_Rups e) => new()
    {
        ID = e.ID,
