...
public class GetAllEduSpecialitiesQueryHandler : IRequestHandler<GetAllEduSpecialitiesQuery, IReadOnlyList<Edu_SpecialitiesDto>>
{
    private readonly ISsoRepository<Edu_Specialities> _repository;

    public GetAllEduSpecialitiesQueryHandler(ISsoRepository<Edu_Specialities> repository)
    {
        _repository = repository;
    }

    public async Task<IReadOnlyList<Edu_SpecialitiesDto>> Handle(GetAllEduSpecialitiesQuery request, CancellationToken cancellationToken)
    {
        var entities = await _repository.GetAllWithIncludesAsync(
            new[] { "Level", "PrimarySubject", "FithSubject", "RupEditorOrgUnit" }, cancellationToken);

        return entities.Select(MapToDto).ToList().AsReadOnly();
    }

    private static Edu_SpecialitiesDto MapToDto(Edu_Specialities e) => new()
    {
        ID = e.ID,
        Code = e.Code,
        Title = e.Title,
